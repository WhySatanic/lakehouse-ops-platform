...
release-readiness, release-candidate, and Iceberg metadata producers validate their
emitted reports this way, establishing the migration path for other reports.
...
Version `1.8.0` adds runtime schema validation to the Iceberg metadata collector. It
preserves contract `1.0.0` and does not change existing report fields. Because `--schema`
already selects the Iceberg database schema, automation overrides the JSON contract with
the distinct `--report-schema` option.
