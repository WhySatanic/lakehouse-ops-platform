...
release-readiness, release-candidate, Iceberg metadata, and maintenance-plan producers
validate their emitted reports this way, establishing the migration path for other reports.
...
Version `1.9.0` adds runtime schema validation to the Iceberg maintenance planner. It
preserves contract `1.0.0`; the normalized Python representation now uses JSON-native
arrays for checks and actions, matching the unchanged serialized CLI output.
