...
Trino compaction-comparison, and partition-pruning producers validate their emitted
reports this way, establishing the migration path for other reports.
...
Version `1.12.0` adds runtime schema validation to Trino partition-pruning experiments
and an explicit `--report-schema` override. It preserves contract `1.0.0` and the existing
report shape.
