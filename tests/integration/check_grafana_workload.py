...
DASHBOARD_UID = "lakehouse-trino-workload"
DASHBOARD_TITLE = "Lakehouse Trino Workload"
DATASOURCE_UID = "prometheus"
STARTED_QUERIES = "trino_execution_name_QueryManager_StartedQueries"
...
def main() -> int:
    ...
    last_error = "Grafana workload resources are not ready"
    ...
    for _ in range(attempts):
        try:
            ...
            up = _request_json(
                f"{prometheus}/api/v1/query?{urlencode({'query': 'up{job="trino-workload"}'})}"
            )
            started = _request_json(
                f"{prometheus}/api/v1/query?{urlencode({'query': STARTED_QUERIES})}"
            )
            ...
            workload_ok = sample_at_least(started, 1)
            if dashboard_ok and datasource_ok and target_ok and workload_ok:
                print("Grafana workload dashboard has live Trino query metrics")
                return 0
            last_error = (
                f"dashboard_ok={dashboard_ok}, datasource_ok={datasource_ok}, "
                f"target_ok={target_ok}, workload_ok={workload_ok}"
            )
        except (OSError, ValueError, KeyError, TypeError) as error:
            last_error = str(error)
        time.sleep(delay)

    print(f"Grafana workload check failed: {last_error}", file=sys.stderr)
    return 1
